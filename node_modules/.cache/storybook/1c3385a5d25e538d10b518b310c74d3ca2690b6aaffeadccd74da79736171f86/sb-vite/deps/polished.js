...
//#endregion
//#region node_modules/polished/dist/polished.esm.js
function last() {
	var _ref;
...
function calculate(expression, additionalSymbols) {
	...
	do {
		...
		if (bad || (afterValue ? notAfterValue : notNewValue)) throw new PolishedError(37, match ? match.index : expression.length, expression);
		if (afterValue) {
	...
	else if (match) throw new PolishedError(40, match ? match.index : expression.length, expression);
...
function math(formula, additionalSymbols) {
	...
	return "" + calculate(reverseString(reversedFormula.replace(unitRegExp, "")), additionalSymbols) + (formulaMatch ? reverseString(formulaMatch[0]) : "");
}
...
function cssVar(cssVariable, defaultValue) {
	if (!cssVariable || !cssVariable.match(cssVariableRegex)) throw new PolishedError(73);
	var variableValue;
	/* istanbul ignore next */
	if (typeof document !== "undefined" && document.documentElement !== null) variableValue = getComputedStyle(document.documentElement).getPropertyValue(cssVariable);
...
function generateStyles(property, valuesWithDefaults) {
	var styles = {};
	for (var i = 0; i < valuesWithDefaults.length; i += 1) if (valuesWithDefaults[i] || valuesWithDefaults[i] === 0) styles[generateProperty(property, positionMap$1[i])] = valuesWithDefaults[i];
	return styles;
}
/**
...
function directionalProperty(property) {
	...
	var firstValue = values[0], _values$ = values[1], secondValue = _values$ === void 0 ? firstValue : _values$, _values$2 = values[2], thirdValue = _values$2 === void 0 ? firstValue : _values$2, _values$3 = values[3];
	return generateStyles(property, [
		firstValue,
		secondValue,
		thirdValue,
		_values$3 === void 0 ? secondValue : _values$3
	]);
}
/**
...
function important(styleBlock, rules) {
	if (typeof styleBlock !== "object" || styleBlock === null) throw new PolishedError(75, typeof styleBlock);
	var newStyleBlock = {};
	Object.keys(styleBlock).forEach(function(key) {
		if (typeof styleBlock[key] === "object" && styleBlock[key] !== null) newStyleBlock[key] = important(styleBlock[key], rules);
...
function modularScale(steps, base, ratio) {
	...
	if (typeof steps !== "number") throw new PolishedError(42);
	if (typeof ratio === "string" && !ratioNames[ratio]) throw new PolishedError(43);
	var _ref = typeof base === "string" ? getValueAndUnit(base) : [base, ""], realBase = _ref[0], unit = _ref[1];
	var realRatio = typeof ratio === "string" ? getRatio(ratio) : ratio;
	if (typeof realBase === "string") throw new PolishedError(44, base);
	return "" + realBase * Math.pow(realRatio, steps) + (unit || "");
}
var rem$1 = pixelsto("rem");
var defaultFontSize = 16;
function convertBase(base) {
	var deconstructedValue = getValueAndUnit(base);
	if (deconstructedValue[1] === "px") return parseFloat(base);
	if (deconstructedValue[1] === "%") return parseFloat(base) / 100 * defaultFontSize;
...
function remToPx(value, base) {
	...
	if (deconstructedValue[1] !== "rem" && deconstructedValue[1] !== "") throw new PolishedError(77, deconstructedValue[1]);
...
function between(fromSize, toSize, minScreen, maxScreen) {
	...
	if (typeof unitlessMinScreen !== "number" || typeof unitlessMaxScreen !== "number" || !minScreenUnit || !maxScreenUnit || minScreenUnit !== maxScreenUnit) throw new PolishedError(47);
	if (typeof unitlessFromSize !== "number" || typeof unitlessToSize !== "number" || fromSizeUnit !== toSizeUnit) throw new PolishedError(48);
	if (fromSizeUnit !== minScreenUnit || toSizeUnit !== maxScreenUnit) throw new PolishedError(76);
	var slope = (unitlessFromSize - unitlessToSize) / (unitlessMinScreen - unitlessMaxScreen);
	return "calc(" + (unitlessToSize - slope * unitlessMaxScreen).toFixed(2) + (fromSizeUnit || "") + " + " + (100 * slope).toFixed(2) + "vw)";
}
/**
...
function fluidRange(cssProp, minScreen, maxScreen) {
	if (minScreen === void 0) minScreen = "320px";
	if (maxScreen === void 0) maxScreen = "1200px";
	if (!Array.isArray(cssProp) && typeof cssProp !== "object" || cssProp === null) throw new PolishedError(49);
	if (Array.isArray(cssProp)) {
		...
		for (var _iterator = _createForOfIteratorHelperLoose(cssProp), _step; !(_step = _iterator()).done;) {
			...
			var obj = _step.value;
			if (!obj.prop || !obj.fromSize || !obj.toSize) throw new PolishedError(50);
		...
		if (!cssProp.prop || !cssProp.fromSize || !cssProp.toSize) throw new PolishedError(51);
...
function generateFileReferences(fontFilePath, fileFormats, formatHint) {
	if (isDataURI(fontFilePath)) return "url(\"" + fontFilePath + "\")" + generateFormatHint(fileFormats[0], formatHint);
	return fileFormats.map(function(format) {
		return "url(\"" + fontFilePath + "." + format + "\")" + generateFormatHint(format, formatHint);
	}).join(", ");
}
function generateLocalReferences(localFonts) {
	return localFonts.map(function(font) {
		return "local(\"" + font + "\")";
	}).join(", ");
}
function generateSources(fontFilePath, localFonts, fileFormats, formatHint) {
	...
	if (fontFilePath) fontReferences.push(generateFileReferences(fontFilePath, fileFormats, formatHint));
...
function fontFace(_ref) {
	var fontFamily = _ref.fontFamily, fontFilePath = _ref.fontFilePath, fontStretch = _ref.fontStretch, fontStyle = _ref.fontStyle, fontVariant = _ref.fontVariant, fontWeight = _ref.fontWeight, _ref$fileFormats = _ref.fileFormats, fileFormats = _ref$fileFormats === void 0 ? [
		...
		"ttf",
		"svg"
	] : _ref$fileFormats, _ref$formatHint = _ref.formatHint, formatHint = _ref$formatHint === void 0 ? false : _ref$formatHint, _ref$localFonts = _ref.localFonts, localFonts = _ref$localFonts === void 0 ? [fontFamily] : _ref$localFonts, unicodeRange = _ref.unicodeRange, fontDisplay = _ref.fontDisplay, fontVariationSettings = _ref.fontVariationSettings, fontFeatureSettings = _ref.fontFeatureSettings;
	if (!fontFamily) throw new PolishedError(55);
	if (!fontFilePath && !localFonts) throw new PolishedError(52);
	if (localFonts && !Array.isArray(localFonts)) throw new PolishedError(53);
	if (!Array.isArray(fileFormats)) throw new PolishedError(54);
	var fontFaceDeclaration = { "@font-face": {
		fontFamily,
...
function linearGradient(_ref) {
	...
	if (!colorStops || colorStops.length < 2) throw new PolishedError(56);
	return {
		backgroundColor: fallback || colorStops[0].replace(/,\s+/g, ",").split(" ")[0].replace(/,(?=\S)/g, ", "),
		backgroundImage: constructGradientValue(_templateObject$1 || (_templateObject$1 = _taggedTemplateLiteralLoose([
			"linear-gradient(",
			"",
			")"
...
function normalize() {
	...
	return [(_ref = {
		...
		fontFamily: "monospace, monospace",
		fontSize: "1em"
	}, _ref.small = { fontSize: "80%" }, _ref["sub,\n    sup"] = {
		fontSize: "75%",
		lineHeight: "0",
		position: "relative",
		verticalAlign: "baseline"
	}, _ref.sub = { bottom: "-0.25em" }, _ref.sup = { top: "-0.5em" }, _ref.img = { borderStyle: "none" }, _ref["button,\n    input,\n    optgroup,\n    select,\n    textarea"] = {
		fontFamily: "inherit",
		fontSize: "100%",
		lineHeight: "1.15",
		margin: "0"
		...
		borderStyle: "none",
		padding: "0"
	}, _ref["button:-moz-focusring,\n    [type=\"button\"]:-moz-focusring,\n    [type=\"reset\"]:-moz-focusring,\n    [type=\"submit\"]:-moz-focusring"] = { outline: "1px dotted ButtonText" }, _ref.fieldset = { padding: "0.35em 0.625em 0.75em" }, _ref.legend = {
		...
		padding: "0",
		whiteSpace: "normal"
	}, _ref.progress = { verticalAlign: "baseline" }, _ref.textarea = { overflow: "auto" }, _ref["[type=\"checkbox\"],\n    [type=\"radio\"]"] = {
		boxSizing: "border-box",
		padding: "0"
	}, _ref["[type=\"number\"]::-webkit-inner-spin-button,\n    [type=\"number\"]::-webkit-outer-spin-button"] = { height: "auto" }, _ref["[type=\"search\"]"] = {
		WebkitAppearance: "textfield",
		outlineOffset: "-2px"
	}, _ref["[type=\"search\"]::-webkit-search-decoration"] = { WebkitAppearance: "none" }, _ref["::-webkit-file-upload-button"] = {
		WebkitAppearance: "button",
		font: "inherit"
	}, _ref.details = { display: "block" }, _ref.summary = { display: "list-item" }, _ref.template = { display: "none" }, _ref["[hidden]"] = { display: "none" }, _ref), { "abbr[title]": { textDecoration: "underline dotted" } }];
}
var _templateObject;
/**
...
function radialGradient(_ref) {
	...
	if (!colorStops || colorStops.length < 2) throw new PolishedError(57);
	return {
		backgroundColor: fallback || colorStops[0].split(" ")[0],
		backgroundImage: constructGradientValue(_templateObject || (_templateObject = _taggedTemplateLiteralLoose([
			"radial-gradient(",
			"",
			"",
			"",
...
function retinaImage(filename, backgroundSize, extension, retinaFilename, retinaSuffix) {
	...
	if (!filename) throw new PolishedError(58);
	var ext = extension.replace(/^\./, "");
	var rFilename = retinaFilename ? retinaFilename + "." + ext : "" + filename + retinaSuffix + "." + ext;
	return _ref = { backgroundImage: "url(" + filename + "." + ext + ")" }, _ref[hiDPI()] = _extends({ backgroundImage: "url(" + rFilename + ")" }, backgroundSize ? { backgroundSize } : {}), _ref;
}
var functionsMap = {
...
function triangle(_ref) {
	...
	if (isNaN(heightAndUnit[0]) || isNaN(widthAndUnit[0])) throw new PolishedError(60);
	return _extends({
		width: "0",
		height: "0",
...
function hslToRgb(hue, saturation, lightness, convert) {
	if (convert === void 0) convert = convertToInt;
...
function parseToRgb(color) {
	...
	if (hslMatched) {
		var rgbColorString = "rgb(" + hslToRgb(parseInt("" + hslMatched[1], 10), parseInt("" + hslMatched[2], 10) / 100, parseInt("" + hslMatched[3], 10) / 100) + ")";
	...
	if (hslaMatched) {
		var _rgbColorString = "rgb(" + hslToRgb(parseInt("" + hslaMatched[1], 10), parseInt("" + hslaMatched[2], 10) / 100, parseInt("" + hslaMatched[3], 10) / 100) + ")";
...
function rgbToHsl(color) {
	...
	if (max === min) if (color.alpha !== void 0) return {
		hue: 0,
		saturation: 0,
		lightness,
		alpha: color.alpha
	...
	var hue;
	var delta = max - min;
	var saturation = lightness > .5 ? delta / (2 - max - min) : delta / (max + min);
	switch (max) {
		case red:
...
function hsl(value, saturation, lightness) {
	...
	else if (typeof value === "object" && saturation === void 0 && lightness === void 0) return hslToHex(value.hue, value.saturation, value.lightness);
...
function hsla(value, saturation, lightness, alpha) {
	...
	else if (typeof value === "object" && saturation === void 0 && lightness === void 0 && alpha === void 0) return value.alpha >= 1 ? hslToHex(value.hue, value.saturation, value.lightness) : "rgba(" + hslToRgb(value.hue, value.saturation, value.lightness) + "," + value.alpha + ")";
...
function rgb(value, green, blue) {
	...
	else if (typeof value === "object" && green === void 0 && blue === void 0) return reduceHexValue$1("#" + numberToHex(value.red) + numberToHex(value.green) + numberToHex(value.blue));
...
function rgba(firstValue, secondValue, thirdValue, fourthValue) {
	...
	} else if (typeof firstValue === "number" && typeof secondValue === "number" && typeof thirdValue === "number" && typeof fourthValue === "number") return fourthValue >= 1 ? rgb(firstValue, secondValue, thirdValue) : "rgba(" + firstValue + "," + secondValue + "," + thirdValue + "," + fourthValue + ")";
	else if (typeof firstValue === "object" && secondValue === void 0 && thirdValue === void 0 && fourthValue === void 0) return firstValue.alpha >= 1 ? rgb(firstValue.red, firstValue.green, firstValue.blue) : "rgba(" + firstValue.red + "," + firstValue.green + "," + firstValue.blue + "," + firstValue.alpha + ")";
...
var curriedAdjustHue$1 = curry(adjustHue);
/**
...
var curriedDesaturate$1 = curry(desaturate);
/**
...
function getLuminance(color) {
	...
	return parseFloat((.2126 * r + .7152 * g + .0722 * b).toFixed(3));
}
/**
...
function getContrast(color1, color2) {
	...
	return parseFloat((luminance1 > luminance2 ? (luminance1 + .05) / (luminance2 + .05) : (luminance2 + .05) / (luminance1 + .05)).toFixed(2));
}
/**
...
function hslToColorString(color) {
	if (typeof color === "object" && typeof color.hue === "number" && typeof color.saturation === "number" && typeof color.lightness === "number") {
		if (color.alpha && typeof color.alpha === "number") return hsla({
			hue: color.hue,
...
function mix(weight, color, otherColor) {
	...
	var color1 = _extends({}, parsedColor1, { alpha: typeof parsedColor1.alpha === "number" ? parsedColor1.alpha : 1 });
	var parsedColor2 = parseToRgb(otherColor);
	var color2 = _extends({}, parsedColor2, { alpha: typeof parsedColor2.alpha === "number" ? parsedColor2.alpha : 1 });
	var alphaDelta = color1.alpha - color2.alpha;
	var x = parseFloat(weight) * 2 - 1;
	var weight1 = ((x * alphaDelta === -1 ? x : x + alphaDelta) / (1 + x * alphaDelta) + 1) / 2;
...
function readableColor(color, returnIfLightColor, returnIfDarkColor, strict) {
	if (returnIfLightColor === void 0) returnIfLightColor = defaultReturnIfLightColor;
	if (returnIfDarkColor === void 0) returnIfDarkColor = defaultReturnIfDarkColor;
	if (strict === void 0) strict = true;
	var isColorLight = getLuminance(color) > .179;
	var preferredReturnColor = isColorLight ? returnIfLightColor : returnIfDarkColor;
	if (!strict || getContrast(color, preferredReturnColor) >= 4.5) return preferredReturnColor;
...
function rgbToColorString(color) {
	if (typeof color === "object" && typeof color.red === "number" && typeof color.green === "number" && typeof color.blue === "number") {
		if (typeof color.alpha === "number") return rgba({
			red: color.red,
			green: color.green,
			blue: color.blue,
			alpha: color.alpha
...
var curriedSaturate$1 = curry(saturate);
/**
...
var curriedSetHue$1 = curry(setHue);
/**
...
var curriedSetLightness$1 = curry(setLightness);
/**
...
var curriedSetSaturation$1 = curry(setSaturation);
/**
...
var curriedShade$1 = curry(shade);
/**
...
function animation() {
	...
	if (!multiMode && args.length > 8) throw new PolishedError(64);
	return { animation: args.map(function(arg) {
		if (multiMode && !Array.isArray(arg) || !multiMode && Array.isArray(arg)) throw new PolishedError(65);
		if (Array.isArray(arg) && arg.length > 8) throw new PolishedError(66);
...
function backgrounds() {
	for (var _len = arguments.length, properties = new Array(_len), _key = 0; _key < _len; _key++) properties[_key] = arguments[_key];
	return { background: properties.join(", ") };
}
var sideMap = [
	"top",
	"right",
...
function borderRadius(side, radius) {
	...
	if (!radius && radius !== 0) throw new PolishedError(62);
...
function statefulSelectors(states, template, stateMap) {
	...
	for (var i = 0; i < states.length; i += 1) {
		if (stateMap && stateMap.indexOf(states[i]) < 0) throw new PolishedError(68);
...
function position(firstValue) {
	for (var _len = arguments.length, values = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) values[_key - 1] = arguments[_key];
	if (positionMap.indexOf(firstValue) >= 0 && firstValue) return _extends({}, directionalProperty.apply(void 0, [""].concat(values)), { position: firstValue });
	else return directionalProperty.apply(void 0, ["", firstValue].concat(values));
}
/**
...
function transitions() {
	...
	if (Array.isArray(properties[0]) && properties.length === 2) {
		...
		if (typeof value !== "string") throw new PolishedError(61);
		return { transition: properties[0].map(function(property) {
			return property + " " + value;
		}).join(", ") };
	} else return { transition: properties.join(", ") };
}
//#endregion
